...
export default function AdminEditGalleryPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const album = dummyGalleryAlbums.find((a) => a.id === Number(params.id));
    if (album) {
      setForm({ title: album.title, description: album.description || "", date: album.date });
    }
    setIsLoading(false);
  }, [params.id]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      showToast("앨범이 수정되었습니다. (데모)", "success");
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="text-2xl font-bold text-warm-gray-700 mb-6">앨범 수정</h1>
      <Card hover={false}>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-warm-gray-700 mb-1">사진 관리</label>
            <div className="border-2 border-dashed border-warm-gray-300 rounded-lg p-8 text-center text-warm-gray-500">
              ...
              <p className="text-sm">추가 사진 업로드</p>
